  ...
  settings = <<SETTINGS
    {
        ...
        "commandToExecute": "bash masterPrep.sh ${azurerm_storage_account.osstoragepv.name} ${var.openshift_azure_vm_username} && bash deployOpenShift.sh ${var.openshift_azure_vm_username} ${var.openshift_initial_password} ${base64encode(file(var.openshift_azure_private_key))} '${var.openshift_azure_resource_prefix}-vm-master-${var.openshift_azure_resource_suffix}' ${azurerm_public_ip.osmasterip.fqdn} '${var.openshift_azure_resource_prefix}-vm-infra-${var.openshift_azure_resource_suffix}' ${azurerm_public_ip.osinfraip.ip_address} '${var.openshift_azure_resource_prefix}-vm-node-${var.openshift_azure_resource_suffix}' ${var.openshift_azure_node_vm_count} ${var.openshift_azure_infra_vm_count} ${var.openshift_azure_master_vm_count} ${azurerm_public_ip.osinfraip.ip_address}.${var.openshift_azure_default_subdomain} ${azurerm_storage_account.osstorageregistry.name} ${azurerm_storage_account.osstorageregistry.primary_access_key} ${var.azure_tenant_id} ${var.azure_subscription_id} ${var.azure_client_id} ${var.azure_client_secret} ${azurerm_resource_group.osrg.name} '${azurerm_resource_group.osrg.location}' ${azurerm_storage_account.osstoragepv.name} ${azurerm_storage_account.osstoragepv.primary_access_key} ${var.openshift_ansible_url} ${var.openshift_ansible_branch}"
    }
SETTINGS
}
